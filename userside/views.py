...
conn = mcdb.connect(host="localhost", user="root", passwd="p@$$word_SQL", database='myproject1')
...
def selfDefence(request):
    return render(request, 'classDetails/selfDefence.html')

def psychoTraining(request):
    return render(request, 'classDetails/psychoTraining.html')

def fitnessForMan(request):
    return render(request, 'classDetails/fitnessForMan.html')

def strengthTraining(request):
    return render(request, 'classDetails/strengthTraining.html')

def cardioForMan(request):
    return render(request, 'classDetails/cardioForMan.html')

def advanceGym(request):
    return render(request, 'classDetails/advanceGym.html')
def classes(request):
...
def registerprocess(request):
    if request.method == 'POST':
        ...
        # Save files
        try:
            img_path = os.path.join("Adminapp/static/upload/", img.name)
            with open(img_path, 'wb') as f:
                for chunk in img.chunks():
                    f.write(chunk)

            id_proof_path = os.path.join("Adminapp/static/upload/", id_proof.name)
        ...
        # Insert user data
        cur.execute("""
            INSERT INTO user_master (Type_Id, User_Name, Gender, Email, Password, Address, Mobile, Photo, ID_Proof) 
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (typeid, name, gender, email, password, address, mobile, img.name, id_proof.name))
        ...
        months_duration = int(plan)  # Assuming plan is a number representing months
        e_date = s_date + datetime.timedelta(days=30 * months_duration)  # Approximate month length
